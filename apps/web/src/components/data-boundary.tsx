 ...
 * (Sentry via telemetry). There is no silent fallback anywhere in this file.
 */
...
export function DataBoundary<T>({
  ...
  useEffect(() => {
    if (violated) {
      reportError(new Error(`DataBoundary(${name}): API payload missing provenance meta`), {
        boundary: name,
        payload: query.data,
      });
    }
  }, [violated, name, query.data]);
