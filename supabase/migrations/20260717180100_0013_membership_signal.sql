-- Phase 1 · unit 9 — correct recurring-member derivation from Glofox's
-- authoritative membership record. Recent subscription payments corroborate
-- the membership state; they neither qualify nor disqualify a member.

alter table public.people
  add column if not exists membership_type text,
  add column if not exists membership_status text,
  add column if not exists user_membership_id text,
  add column if not exists membership_started_at timestamptz;

create index if not exists people_tenant_membership_status_idx
  on public.people (tenant_id, membership_status)
...
comment on function app.recompute_person_relationship(uuid, uuid) is
  'SERVICE-ROLE ONLY. Atomically derives effective-dated relationship facts and the one KPI primary relationship using rule version 2 membership-record evidence.';
