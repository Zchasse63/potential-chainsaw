...
| **R0.7 credits reconciliation + rebuild the stale ledger** | **Verified 2026-07-22 (see below):** the credit_ledger is missing 41 members / 331 outstanding credits (incl. all 4 owners) — a STALE ledger, not a schema bug (the `membership_id` contract is already `.nullish()`). Fix: (1) run `glofox.sync.credits` to COMPLETION over all 1,366 members (idempotent insert writes the 40 missing grants + reconstructs debits from `bookings[]` — approach A, automatic); (2) re-sync the 1 stale membership plan; (3) add credits to RECONCILE_ENTITIES (Kelo-vs-Glofox balance diff per person) with explicit expiry semantics; the pre-flip credit attestation. | workers/src/glofox/entities/credits.ts (O(members) self-re-enqueue); reconcile.ts:76-81 |
| **R0.8b import-integrity guards** _(the generalizable lesson from the credit bug)_ | Two systemic defects let the credit loss stay invisible: (a) `strictRow` hardcodes `external_ref = NULL` on every parse-failure quarantine (workers/src/glofox/entities/shared.ts:24) → quarantine rows can't be joined to the member/credit/txn they represent; extract the raw `_id` into external_ref. (b) No tripwire: a run that fetches 305, quarantines 71, derives 234 reports `success` silently (pipeline.ts alerts only on zero-fetch / below-min). Add a per-run quarantine-rate alarm AND a raw→derived conservation check (raw_count = derived + still-quarantined, per entity) into reconciliation + /health — the check that flips this whole class from silent to red. Also: a `resolved` quarantine row must be verified present in its derived table, never bulk-flagged. | shared.ts:24; pipeline.ts |
