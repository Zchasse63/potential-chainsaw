...
| Unit | Scope | Key refs |
|---|---|---|
| **R0.1 cadence producer** | The missing "unit 1.7": seed `glofox.sync.all` per active tenant from the tick (in-tick hour-keyed enqueue is sufficient — the fan-out's idempotency buckets already make double-fire safe; a schedule table is acceptable if cleaner). Test: a cold queue self-populates. **Constraint (from the 2026-07-23 credit rebuild):** the credits self-chain's per-chunk re-enqueue key `glofox.sync.credits:<tenant>:after:<cursor>` (entities/credits.ts:235) is stable across runs, so a repeated full walk dedupes every chunk past chunk 1 against the prior cycle's completed jobs → the walk silently stops at 500 members every cycle after the first. The producer + chain MUST thread a per-CYCLE nonce (e.g. the hour bucket) through the job payload into the chunk idempotency keys, so each cycle is a fresh chain while retries WITHIN a cycle still dedupe. **Chain half DONE — merged 2026-07-23 (commit af7eb38):** the fan-out (`glofox.sync.all`) now passes `{ cycle: hourBucket }` in each sync payload, and credits `afterSuccess` threads `cycle` into both the next chunk's payload and its idempotency key (`...:<cycle>:after:<cursor>`); a second-cycle test asserts two cycles produce distinct chunk keys. Manual/ad-hoc full re-walks must still pass a distinct `cycle` (or drive explicit keys, as the 2026-07-23 rebuild did) — the `"adhoc"` fallback is stable by design so lease-expiry retries dedupe. **STILL OPEN — the actual producer:** nothing seeds `glofox.sync.all` from the tick, so the live cadence remains FROZEN. That is the remaining R0.1 work. | workers/src/glofox/processors.ts:301-315; workers/src/glofox/entities/credits.ts:229-243; netlify/functions/scheduler-tick.mts |
...
| **R0.7 credits reconciliation + rebuild the stale ledger** | **Ledger rebuild DONE — verified 2026-07-23.** Ran `glofox.sync.credits` to completion over all 1,366 members (via `runTick`, the real vetted path); recovered the 41 missing members / 331 outstanding credits, incl. the 4 owner member-accounts (Loring 89, Ferrell 78, Chasse 8, G. Pages 5). `credit_ledger` now reconciles EXACTLY to the raw zone: **307 grants = 307 active packs, 1,907 granted, 1,199 outstanding (= `app.credit_balances` matview total), 708 consumed; 0 missing packs, 0 new quarantine.** Root cause = a STALE ledger (the pre-fix stricter `membership_id` contract quarantined these packs; contract is already `.nullish()`), NOT a schema bug. **Distinct defect found in the process (see R0.1):** the credits self-chain is not re-runnable — its `after:<cursor>` re-enqueue key is stable across runs, so a repeated full walk dedupes every chunk past chunk 1 against the prior cycle's completed jobs (this is why a plain re-run only touched members 1–500). Worked around during the rebuild by driving the cursor chunks with fresh keys; the chain is now made re-runnable in code (see R0.1, commit af7eb38). **Follow-ups DONE — merged 2026-07-23 (commit af7eb38):** (2) the 1 stale membership plan re-synced — the `max_group_membership_size` contract relaxed to `.nullish()` (a real active plan omits the key; same over-strict-contract class), memberships re-synced (catalog 24→25, gap 0), its stale open quarantine row verified-present-then-resolved; (3) credits added to RECONCILE_ENTITIES as `checkCredits` (see R0.8b) — the standing per-person credit attestation, verified live `match`. **R0.7 is fully closed.** | workers/src/glofox/entities/credits.ts; reconcile.ts; packages/contracts/src/glofox/memberships.ts |
| **R0.8b import-integrity guards** _(the generalizable lesson from the credit bug)_ | **(a)+(b) DONE — merged 2026-07-23 (commit 28b1181).** (a) `strictRow` now attributes every parse-failure quarantine row to its record (`external_ref = _id`, else `id`), so `import_quarantine` is join-able to the member/credit/txn it stands for (workers/src/glofox/entities/shared.ts). (b) Pipeline **Tripwire 5**: any `success` run with `quarantined > 0` opens a deduped `sync_quarantine` warning that stays open until the queue is worked — the silent-success class (a run fetches N, quarantines M, derives N−M, reports `success`) is now loud (pipeline.ts). 3 tests + independent review; all 248 workers tests green. **Conservation check DONE — merged 2026-07-23 (commit af7eb38):** `checkCredits` in reconcile.ts is a DB-only raw→ledger conservation check (glofox_raw /2.0/credits latest-pack-per-`_id` vs `credit_ledger` where source='glofox') — attests grants == raw packs, outstanding reconciles, and per-person balances match; drifts loud otherwise. It surfaces the STANDING gap Tripwire 5 can't (Tripwire 5 only fires on NEW quarantine in a run; the 124 credit rows were all marked `resolved`, so /health's open count read 0). Writes a `reconciliations` row that /health already renders; verified live 2026-07-23: status `match`, drift 0. Also: a `resolved` quarantine row must be verified present in its derived table before flagging, never bulk-flagged (Tripwire 5's alert body encodes this; the membership-plan resolve on 2026-07-23 followed it). **Follow-on (optional):** extend the same raw→derived conservation to members/bookings/transactions (they have live-API reconcile already; only credits lacked a standing check). | shared.ts; pipeline.ts; reconcile.ts; apps/api/src/routes/health.ts |
