...
| **Trust streak** | Consecutive days with zero **failed-check** figures rendered and zero unverified figures shown **unmarked**. The verification-state trio (canonical, shared with the design guide): *verified* renders plain; *imported-unverified* renders marked (hatch + trailing `*`) and does **not** break the streak — marked honesty is the point; *failed-check* is greyed out with a reason and never renders as a plain number — rendering one plain breaks the streak. Computed from the verification ledger. | Refined 2026-07-17 (external audit caught the three states conflated). The Health page's headline number — the founding trauma, inverted into a metric. |
...
live depth probe, see docs/glofox/README.md §5) and **1,366 members** —
