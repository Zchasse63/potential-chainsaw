...
export default tseslint.config(
  {
    ignores: [
      ...
      "**/dist-test/**",
      "**/node_modules/**",
      // Generated by @netlify/vite-plugin-tanstack-start on apps/member builds.
      "**/.netlify/**",
      "supabase/.temp/**",
  ...
  // GUARDRAILS 2+3 for apps/web AND apps/member — kept in ONE block because
  // flat config does not merge same-name rules across blocks: a second
  // no-restricted-syntax for the same files would REPLACE this one, silently
  // disarming it (probe-verified 2026-07-17).
  //
  ...
  // reason. The member client ships ZERO Supabase material at all
  // (plan-member-app §5) — the CI `secrets` job greps both apps' sources for
  // the string literals; this rule catches it earlier, at lint time.
  //
  ...
  // lint-blocked"). Both web apps consume design tokens via the shared
  // @kelo/ui tailwind-preset (var(--kelo-*) from packages/ui/tokens/tokens.css)
  // — raw hex color literals and arbitrary Tailwind bracket values
  // ("w-[13px]", "bg-[#fff]") are banned in string literals and JSX text.
  // tokens.css itself is CSS, not linted TS.
  {
    files: [
      "apps/web/**/*.{ts,tsx,js,jsx,mts,mjs,cts,cjs}",
      "apps/member/**/*.{ts,tsx,js,jsx,mts,mjs,cts,cjs}",
    ],
    rules: {
      "no-restricted-syntax": [
        "error",
        {
          ...
          message:
            "SUPABASE_SERVICE_ROLE_KEY bypasses RLS — it must never be referenced from the client web apps (apps/web, apps/member).",
        },
        {
        ...
        },
        {
          ...
          message:
            "SUPABASE_DB_URL is the service-level Postgres pool string — it must never be referenced from the client web apps (apps/web, apps/member).",
        },
        {
        ...
        },
        {
          ...
          message:
            "Raw hex colors are banned in apps/web and apps/member — use the semantic design tokens (Tailwind classes mapped to var(--kelo-*)).",
        },
        {
        ...
        },
        {
          ...
          message:
            "Arbitrary Tailwind values (e.g. w-[13px]) are banned in apps/web and apps/member — use token-backed scale classes from the design system.",
        },
        {
